...
int funcao(int P,int x[P][5], int V[P][5],float U[P][5],float fi1,float fi2, int rep, int H)
{ 
...
int main() 
{
	...
	printf("Defina o parametro cognitivo:(MAX 1,6)");
	scanf("%f",&fi1);
	printf("Defina o parametro social:(MAX 1.6)");
	scanf("%f",&fi2);
	...
	for(H=0;H<rep;H++)
	{
	r=funcao(P,x,V,U,fi1,fi2,rep,H);
	printf("\n REPETICAO (%d) \n",H);
	}
